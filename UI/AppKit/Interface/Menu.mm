...
- (void)execute:(id)sender
{
    ...
    if (![[[NSApp keyWindow] firstResponder] isKindOfClass:[CryFoxWebView class]]) {
        switch (action->id()) {
...
NSMenu* create_context_menu(CryFoxWebView* view, WebView::Menu& menu)
{
    auto* application_menu = create_application_menu(menu);

    __weak CryFoxWebView* weak_view = view;
