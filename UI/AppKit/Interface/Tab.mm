...
@interface Tab () <CryFoxWebViewObserver>
...
+ (NSImage*)defaultFavicon
{
    ...
    dispatch_once(&token, ^{
        ...
        auto* ns_default_favicon_path = CryFox::string_to_ns_string(default_favicon_path->filesystem_path());
...
- (instancetype)init
{
    auto* web_view = [[CryFoxWebView alloc] init:self];
...
{
    auto* web_view = [[CryFoxWebView alloc] initAsChild:self parent:[parent web_view] pageIndex:page_index];
    return [self initWithWebView:web_view];
}
...
#pragma mark - CryFoxWebViewObserver
...
{
    self.title = CryFox::string_to_ns_string(url.serialize());
    self.favicon = [Tab defaultFavicon];
    [self updateTabTitleAndFavicon];
...
{
    self.title = CryFox::utf16_string_to_ns_string(title);
    [self updateTabTitleAndFavicon];
}

- (void)onFaviconChange:(Gfx::Bitmap const&)bitmap
{
    auto* favicon = CryFox::gfx_bitmap_to_ns_image(bitmap);
