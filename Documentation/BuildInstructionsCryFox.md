...
A Nix development shell is maintained [here](https://github.com/nix-community/nix-environments/tree/master/envs/cryfox),
...
Xcode 15 or clang from homebrew is required to successfully build cryfox.
...
```
In a VS command prompt build using cryfox.py:
```
py Meta\cryfox.py build
```
...
The simplest way to build and run cryfox is via the cryfox.py script:
...
```bash
CC=$(brew --prefix llvm)/bin/clang CXX=$(brew --prefix llvm)/bin/clang++ ./Meta/cryfox.py run
```

You may also choose to start it in `gdb` using:
```bash
./Meta/cryfox.py gdb cryfox
```

The above commands will build a Release version of CryFox. To instead build a Debug version, run the
`Meta/cryfox.py` script with the value of the `BUILD_PRESET` environment variable set to `Debug`, like this:

```bash
BUILD_PRESET=Debug ./Meta/cryfox.py run
```
...
`./Meta/cryfox.py run <executable_name>`.
...
CryFox requires resource files from the cryfox/Base/res directory in order to properly load
...
The script Meta/cryfox.py and the default preset in CMakePresets.json both define a build directory of
...
Note that when using a custom build directory rather than Meta/cryfox.py, the user may need to provide a suitable C++
compiler (see [Build Prerequisites](BuildInstructionsCryFox.md#build-prerequisites)) via the CMAKE_C_COMPILER and
...
The Meta/cryfox.py script will execute the `run-cryfox` and `debug-cryfox` custom targets.
If you don't want to use the cryfox.py script to run the application, you can run the following commands:

To automatically run in gdb:
```
ninja -C Build/release debug-cryfox
```
...
open -W --stdout $(tty) --stderr $(tty) ./Build/release/bin/CryFox.app --args https://cryfox.dev
```
...
After running CryFox as suggested above with `./Meta/cryfox.py run cryfox`, you can now in CLion use Run -> Attach to Process to connect. If debugging layout or rendering issues, filter the listing that opens for `WebContent` and attach to that.
...
Simply run the `cryfox.py` script as normal, and then make sure to codesign the CryFox binary with the proper entitlements to allow Instruments to attach to it.

```
./Meta/cryfox.py build
