...
function saveEngine(type) {
    ...
    if (engine.selectedIndex !== 0) {
        cryfox.sendMessage(`set${name}Engine`, engine.value);
    } else {
        cryfox.sendMessage(`set${name}Engine`, null);
    }
}
...
function showSearchEngineSettings() {
    ...
    custom.forEach(custom => {
        ...
        remove.addEventListener("click", () => {
            cryfox.sendMessage("removeCustomSearchEngine", custom);
        });
...
function addCustomSearchEngine() {
    ...
    cryfox.sendMessage("addCustomSearchEngine", {
...
document.addEventListener("WebUILoaded", () => {
    cryfox.sendMessage("loadAvailableEngines");
});
