...
def main():
    ...
    if platform.host_system != HostSystem.Windows and os.geteuid() == 0:
        print("Do not run cryfox.py as root, your Build directory will become root-owned", file=sys.stderr)
        sys.exit(1)
    elif platform.host_system == HostSystem.Windows and "VCINSTALLDIR" not in os.environ:
        print("cryfox.py must be run from a Visual Studio enabled environment", file=sys.stderr)
        sys.exit(1)

    if "target" in args:
        if platform.host_system != HostSystem.Windows and args.target == "cryfox":
            args.target = "CryFox"
        if not args.target and args.command not in ("build", "rebuild"):
            args.target = "cryfox" if platform.host_system == HostSystem.Windows else "CryFox"
...
def configure_main(platform: Platform, preset: str, cc: str, cxx: str) -> Path:
    cryfox_source_dir, build_preset_dir = configure_build_env(platform, preset)
    build_vcpkg()

    if build_preset_dir.joinpath("build.ninja").exists() or build_preset_dir.joinpath("cryfox.sln").exists():
    ...
    if "Swift" in preset:
        compilers = pick_swift_compilers(platform, cryfox_source_dir)
    ...
    # https://github.com/CryFoxBrowser/cryfox/blob/master/Documentation/BuildInstructionsCryFox.md#unable-to-find-a-build-program-corresponding-to-ninja
...
def configure_skia_jemalloc() -> list[str]:
    ...
    # https://github.com/CryFoxBrowser/cryfox/issues/261
...
def configure_build_env(platform: Platform, preset: str) -> tuple[Path, Path]:
    cryfox_source_dir = ensure_cryfox_source_dir()
    build_root_dir = cryfox_source_dir / "Build"
    ...
    known_presets = {
        ...
        "Performance": build_root_dir / "performance",
        "Legacy": build_root_dir / "legacy",
    }
    ...
    os.environ["PATH"] += os.pathsep + str(cryfox_source_dir.joinpath("Toolchain", "Local", "cmake", "bin"))
    ...
    return cryfox_source_dir, build_preset_dir
...
def ensure_cryfox_source_dir() -> Path:
    cryfox_source_dir = os.environ.get("CRYFOX_SOURCE_DIR", None)
    cryfox_source_dir = Path(cryfox_source_dir) if cryfox_source_dir else None

    if not cryfox_source_dir or not cryfox_source_dir.is_dir():
        root_dir = run_command(["git", "rev-parse", "--show-toplevel"], return_output=True, exit_on_failure=True)
        assert root_dir

        os.environ["CRYFOX_SOURCE_DIR"] = root_dir
        cryfox_source_dir = Path(root_dir)

    return cryfox_source_dir
...
def clean_main(platform: Platform, preset: str):
    ...
    user_vars_cmake_module = cryfox_source_dir.joinpath("Meta", "CMake", "vcpkg", "user-variables.cmake")
